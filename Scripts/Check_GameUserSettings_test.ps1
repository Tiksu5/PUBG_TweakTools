...
# ennen scope-decimal checkiä, tarkista onko universal sens käytössä ja hälytä sen mukaan.
...
foreach ($Keyword in $global:Keywords) {
    $FilteredLines = $FileContent | Where-Object { $_ -notmatch 'TslPersistantData' }
    Write-Host "Filtered Lines:"
    Write-Host $FilteredLines

    $Matches = $FilteredLines | Select-String -Pattern "$Keyword([\d\.]+)"
    Write-Host "Matches:"
    Write-Host $Matches

    $Value = $Matches | ForEach-Object { $_.Matches.Groups[1].Value.Trim() }
    Write-Host "$Keyword Value: $Value"
...
#        if ($Keyword -notin $KeywordsMultipleMatch) {
            $Value = $FileContent | Where-Object { $_ -notmatch 'TslPersistantData' } | 
                Select-String -Pattern "$Keyword([\d\.]+)" | ForEach-Object { $_.Matches.Groups[1].Value.Trim() }
            $global:KeywordValues[$Keyword] = $Value
#        }
